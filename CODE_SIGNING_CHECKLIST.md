# mk_photo_mask V4.6 正式 Code Signing 檢查清單

## 必要條件

1. 一張受 Windows 信任的 Code Signing 憑證，且包含 Code Signing EKU `1.3.6.1.5.5.7.3.3`。
2. 若憑證可匯出為 PFX：需要 PFX 檔與密碼。
3. GitHub repository 必須已放入 V4.6 原始碼與 `.github/workflows/build-windows.yml`。
4. GitHub Actions 需要 `contents: write` 權限，V4.6 workflow 已設定。
...
3. Windows runner 建置 `mk_photo_mask_v4.6.exe`。
