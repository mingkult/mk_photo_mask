...
Write-Host "============================================================"
Write-Host " mk_photo_mask V4.6 - GitHub Publisher"
...
Write-Host "[5/8] Staging V4.6 source..."
& git add -A
if ($LASTEXITCODE -ne 0) { throw "git add failed." }

& git diff --cached --quiet
if ($LASTEXITCODE -ne 0) {
  & git commit -m "Publish mk_photo_mask V4.6"
...
# Preserve the remote history and place the current V4.6 snapshot on top of it
...
if ($remoteBranchExists) {
  ...
  if (-not $remoteIsAncestor) {
    ...
    Write-Host "Preserving the remote history and rebuilding the V4.6 snapshot on top of it..."

    $backupBranch = "local-v4.6-before-sync-" + (Get-Date -Format "yyyyMMdd-HHmmss")
    ...
    # Move HEAD to the remote commit but keep the current V4.6 index and
    # working tree. The next commit therefore becomes a normal child of the
    # existing GitHub history while retaining the exact V4.6 files.
    ...
    Write-Host "Remote history is already an ancestor of the local V4.6 commit."
  }
}
...
Write-Host "  3. Push tag v4.6 to trigger the signed Windows Release workflow."
